...
def run_bulk_ingestion_workflow(
    ...
    """
    Core ingestion algorithm moved here to be called by Celery.
    Processes every file in the temp_dir.
    Updates 'job' object (processed_files) and 'db' incrementally.
    """
    results = []
    processed_files_list = []
    
    # 1. Expand files from temp folder (including sub-zips)
    files_to_read = os.listdir(temp_dir) if os.path.exists(temp_dir) else []
    
    # Prepare all files (recursive expansion of zips if any)
    for filename in files_to_read:
        ...
        if filename.lower().endswith(".zip"):
            try:
                ...
                results.append({"file_name": filename, "status": "error", "error": f"Failed to extract zip: {e}"})
        else:
            processed_files_list.append({
                "filename": filename,
    ...
    # Update total count in Job record
    job.total_files = len(processed_files_list)
    db.commit()

    # 2. Sequential ingestion per dataset
    ...
    for item in processed_files_list:
        current_filename = item["filename"]
        file_content = item["content"]
        
        try:
            # Step 1 — Parse file
            df = load_dataframe(file_content, current_filename)
            if df.empty or len(df.columns) == 0:
                raise Exception("File contains no usable data")

            # Step 2 — Infer schema
            schema_meta = infer_schema(df, product_id, current_filename)
            dataset_meta = schema_meta["dataset"]
            columns_meta = schema_meta["columns"]

            # Step 3 — Create table
            from ..services.dataset_store import create_dataset_table, insert_dataset_rows, drop_dataset_table
            ...
            if auto_map or logical_dataset_name:
                try:
                    ...
                    print(f"DEBUG: Background auto-mapping failed: {e}")
            ...
            results.append({"file_name": current_filename, "status": "error", "error": str(e)})
